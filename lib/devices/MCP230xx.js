...
MCP230xx.prototype.inputAll = function(cb) {
  this._readGPIO(function(value) {
    var result = [];
    for (var i = 0; i < 16; i++) {
      result[i] = !!(value & (1 << i));
    }
...
MCP230xx.prototype._readGPIO = function(cb) {
  var _self = this;
  ...
  step(
    function readRegisterA() {
      var next = this;
      _self._readU8(MCP23017_GPIOA, function(err, lower){
        value += lower;
        next();
      });
    },
    ...
    function readRegisterB() {
      var next = this;
      _self._readU8(MCP23017_GPIOB, function(err, upper){
        value += (upper << 8);
        next();
      });
    },
